...
def update_existing_app_status(app_list):
    for db_entry in App_Store.query.all():
        ...
        if db_entry.name not in app_list:
            ...
            if db_entry.dependencies:
                ...
                for dependency in json_dep:
                    ...
                    try:
                        ...
                        logger.debug(
                            "Pruned container: " f"{json_dep['dependency']}"
                        )
                    except Exception:
                        logger.exception(
                            "Dependency image may already have " "been pruned."
                        )
            ...
            try:
                docker_py.prune(image=db_entry.image, network=db_entry.name)
                logger.debug("Pruned container: " f"{db_entry.image}")
            except Exception:
                logger.exception("Image may already have been pruned.")

            # If a logo was downloaded then remove it
            if db_entry.logo and os.path.exists(
                os.path.realpath(".") + db_entry.logo
            ):
                try:
        ...
        # If the application is installed and there is an update available
            ...
            db_entry.status.lower() == "installed"
            and db_entry.version < app_list[db_entry.name]["version"]
        ):
    ...
    for app in app_list:
        ...
        if logo_path:
            try:
                ...
                r = requests.get(logo_path, stream=True, timeout=5)
