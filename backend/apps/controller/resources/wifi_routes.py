...
class wifi_connect(Resource):
    ...
    def post(self):
        ...
        wifi_connect_thread = threading.Thread(
            target=wifi.connect, kwargs=content
        )
...
class wifi_connection_status(Resource):
    def get(self):
        return {"wifi": check_connection(), "internet": check_internet()}
...
class wifi_forget(Resource):
    ...
    def get(self):
        ...
        wifi_forget_thread = threading.Thread(
            target=wifi.forget,
            kwargs={"create_new_hotspot": True, "all_networks": False},
        )
...
class wifi_forget_all(Resource):
    ...
    def get(self):
        wifi_forget_thread_all = threading.Thread(
            target=wifi.forget_all, args=(1,), name="wifi_forget_thread_all"
        )
