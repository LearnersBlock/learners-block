...
def generate_path(root, path):
    return os.path.join(
        system_root, sanitise(string=root), *sanitise(obj=path)
    )
    ...
    def post(self):
        ...
        for item in content["object"]:
            ...
            if item["format"] == "file":
                ...
                # Copy new item
                shutil.copy2(os.path.join(fromPath, itemName), toPath)
                ...
                # Copy new item
                shutil.copytree(
                    os.path.join(fromPath, itemName),
...
class filemanager_list(Resource):
    def post(self):
        ...
        for direc in os_sorted(next(os.walk(path))[1]):
            output.append({"name": direc, "format": "folder"})

        for f in os_sorted(next(os.walk(path))[2]):
            file_ext = os.path.splitext(f)
            output.append(
                {"name": f, "format": "file", "extension": file_ext[1]}
            )
...
class filemanager_move(Resource):
    ...
    def post(self):
        ...
        for item in content["object"]:
            ...
            shutil.move(os.path.join(fromPath, itemName), toPath)
...
class filemanager_unzip(Resource):
    ...
    def post(self):
        ...
        if os.path.exists(os.path.join(path, Path(filename).stem)):
            i = 1
            while True:
