...
const auth = {
  ...
  mutations: {
    ...
    SET_API: (state: AuthStateInterface, payload: string) => {
      state.api =
        (process.env.DEV as unknown as boolean) === true
  ...
  actions: {
    LOGIN: async (
      ...
      const response = await AxiosOverride.post(
        `${getters.GET_API}/v1/auth/log_in`,
        payload
      ).catch(() => {
    ...
    LOGOUT: async ({ commit, getters }: { commit: any; getters: any }) => {
      const response = await Axios.post(
        `${getters.GET_API}/v1/auth/log_out`
      ).catch((e) => {
    ...
    VERIFY_LOGIN: async (
      {
        ...
        getters,
        dispatch
      }: { commit: any; getters: any; dispatch: any },
      payload: string
    ) => {
      const response = await Axios.get(
        `${getters.GET_API}/v1/auth/verify_login`,
        {
          headers: {
